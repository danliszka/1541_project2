...
int main(int argc, char **argv)
{
  ...
  while(1) {
    ...
    else
    {
      ...
      {
        if (L2_size > 0)
          L1_I_penalty = cache_access(L1_I_CACHE, (unsigned long) IF1->PC, 0, L2_CACHE, NULL);
        else
          L1_I_penalty = cache_access(L1_I_CACHE, (unsigned long) IF1->PC, 0, NULL, NULL);
      ...
      if (MEM1->type == ti_LOAD) //if LOAD instruction
      {
        ...
        if (D_size > 0)
        {
          if (L2_size > 0)//we have L2 cache
            L1_D_penalty = cache_access(L1_D_CACHE, (unsigned long) MEM1->Addr, 0, L2_CACHE, NULL);
          else
            L1_D_penalty = cache_access(L1_D_CACHE, (unsigned long) MEM1->Addr, 0, NULL, NULL);
      ...
      else if (MEM1->type == ti_STORE) //if STORE instruction
      {
        ...
        if (D_size > 0)
        {
          if (L2_CACHE > 0)//we have L2 cache
            L1_D_penalty = cache_access(L1_D_CACHE, (unsigned long) MEM1->Addr, 1, L2_CACHE, NULL);
          else
            L1_D_penalty = cache_access(L1_D_CACHE, (unsigned long) MEM1->Addr, 1, NULL, NULL);
...
int hazardCheck(struct trace_item *IF1inst, struct trace_item *IF2inst, struct trace_item *IDinst, struct trace_item *EXinst, struct trace_item *MEM1inst, struct trace_item *MEM2inst, struct trace_item *WBinst ){
  ...
  if (MEM1inst->type == ti_LOAD ){
   if(EXinst->type == ti_ITYPE && EXinst->sReg_a == MEM1inst->dReg)
    return 2;
  else if(EXinst->type == ti_JRTYPE && EXinst->dReg == MEM1inst->dReg)
    return 2;
  ...
  if (MEM2inst->type == ti_LOAD ){
    if(EXinst->type == ti_ITYPE && EXinst->sReg_a == MEM2inst->dReg)
      return 3;
    else if (EXinst->type == ti_JRTYPE && EXinst->dReg == MEM2inst->dReg)
      return 3;
  ...
  if (WBinst->type == ti_ITYPE || WBinst->type ==ti_RTYPE){
    if(IDinst->type == ti_ITYPE && IDinst->sReg_a == WBinst->dReg)
    return 1;
  else if(IDinst->type == ti_JRTYPE && IDinst->dReg == WBinst->dReg)
    return 1;
